...
export default function AutopilotButton({ track }: { track: TrackId }) {
  ...
  const run = async () => {
    ...
    try {
      ...
      let m = "";
      if (rep.healed) m += `复活卡死 ${rep.healed} 篇；`;
      if (rep.retried.length > 0) m += `重试失败稿 ${rep.retried.filter((x) => x.ok).length}/${rep.retried.length} 成功；`;
      m += `抓取 ${rep.fetched}；${rep.scored}；产出 ${ok.length} 篇待审终稿`;
