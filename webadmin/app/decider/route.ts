 ...
 *  落点是 /guides 教程库面板（用户拍板：从主站进来直接看全部教程，答题漏斗在面板一键可达）。
 *  公网门面实例没配 DECIDER_URL 时回首页（decider 还没上线，别给访客 :3100 断链）。 */
export async function GET(request: Request) {
  const configured = process.env.DECIDER_URL;
  if (configured) return Response.redirect(`${configured.replace(/\/$/, "")}/guides`, 307);
  ...
  return Response.redirect(`http://${host}:3100/guides`, 307);
}
